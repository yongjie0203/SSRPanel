...
class MarketingController extends Controller
{
    ...
    public function read($email_id,$task_id,Request $request){
        ...
        if(!empty($u)){
            //单封单人
           $task_read = DB::table('email_read_log')->selectRaw('count(distinct email_read_log.email) `read`')->where('task_id',$task_id)->get()->toArray()[0]->read;
           $mail_read = DB::table('email_read_log')->selectRaw('count(distinct email_read_log.email) `read`')->where('email_id',$email_id)->get()->toArray()[0]->read;
        }else{
            //单封多人
           $task_read = DB::table('email_read_log')->selectRaw('count(distinct email_read_log.ip) `read`')->where('task_id',$task_id)->get()->toArray()[0]->read;
           $mail_read = DB::table('email_read_log')->selectRaw('count(distinct email_read_log.ip) `read`')->where('email_id',$email_id)->get()->toArray()[0]->read;
        }
