...
/**
 * Class ImeimallPayController
 *
 * @author  zhangyongjie
 *
 * @package App\Http\Controllers\Api
 */
class ImeimallPayController extends Controller
{
    ...
    {
        \Log::info("【iPay】回调接口[GET]：" . var_export($request->all(), true) . '[' . getClientIp() . ']');
        exit("【iPay】接口正常");
    }
    ...
    {
        \Log::info("【iPay】回调接口[POST]：" . var_export($request->all(), true));
        ...
        if ($verify_result) { // 验证成功
            ...
            if ($request->get('status') == 'TRADE_FINISHED' || $request->get('status') == 'SUCCESS') {
                ...
                // 支付宝交易号
                $data['trade_no'] = $request->get('tradeId');
                // 交易状态
                $data['trade_status'] = $request->get('status');
                // 交易金额(这里是按照结算货币汇率的金额，和rmb_fee不相等)
                $data['total_fee'] = $request->get('totalFee');

                $this->tradePaid($data);
            } else {
                Log::info('iPay-POST:交易失败[' . getClientIp() . ']');
            }
        } else {
            Log::info('iPay-POST:验证失败[' . getClientIp() . ']');
        }
    ...
    private function tradePaid($msg)
    {
        ...
        try {
            ...
            DB::rollBack();
            Log::info('【ipay】回调更新支付单和订单异常：' . $e->getMessage());
        }
    }
